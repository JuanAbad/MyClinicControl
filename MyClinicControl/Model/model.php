...
class Model
{
    ...
    public function selectDayMonthAndYear(){
        $query = "select dia,mes,annio from citas";
        $result = $this->db->query(($query));
    ...
    public function selectInformesByDni($dni)
    {
        $model = new Model(); 
        $dnidoctor = $_COOKIE['dniDoctor'];
        $query = "SELECT * FROM informes WHERE dnidoctor='".$dnidoctor."' and paciente like'%$dni%'";
        ...
        $nombreDoctor = $nombreDoctorArray[0];
        
        while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
            $informe = new Informe($row['id'],$row['titulo'],$row['Paciente'], $row['FechaInforme'], $row['Informe']);
            $informe->setPaciente($nombreDoctor);
            $informes[] = $informe;
        }
        return $informes;
    }
