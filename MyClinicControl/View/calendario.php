...
$fechasCitas = $controllerCita->getDayMesAndAnnio();
$jsonFechasCitas = json_encode($fechasCitas);
...
<div class="modal fade custom-modal" id="modalDetalleCita" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false" role="dialog" aria-labelledby="modalTitleId" aria-hidden="true">
    <div class="modal-dialog modal-detalle modal-md" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <h1 class="text-center display-5" id="tituloCita">Revisión craneal</h1>
          <hr>
          <h2 class="text-center display-6" id="horaCita">08:00 - 08:15</h2>
          <br><br>
          <ul class="list-unstyled">
            <li class="display-6 text-sm" id="txtPacienteCita"><strong>Paciente: </strong></li>
            <br>
            <li class="display-6" id="txtDetalleCita"><strong>Detalles: </strong> Lorem ipsum dolor sit amet consectetur adipisicing elit. Obcaecati, eveniet voluptate perspiciatis voluptas sequi eaque animi expedita optio nostrum excepturi. Commodi iste atque saepe enim veritatis animi, blanditiis ducimus repellendus. </li>
            <br>
          </ul>
        </div>
      </div>
...
<div class="container-fluid">
  <h1 class="text-center mt-3 display-5">Mi calendario</h1>
  <br><br><br>
  ...
  <form action="../View/calendario.php" class="w-100" method="post">
  <div class="input-group">
    <input type="text" id="searchInput" name="dnibuscarcitas" style="background-color: #f5f5f5;" class="form-control border-primary rounded-start" placeholder="Buscar citas por DNI...">
    <button class="btn border-primary" type="submit">
      ...
      if(isset($_POST['dnibuscarcitas'])){
        $citas = $controllerCita->getCitasByDni($_POST['dnibuscarcitas']);
      ...
      foreach ($citas as $index =>$cita) :
        ...
        try {
          if (!empty($detallesPaciente) && isset($detallesPaciente[0])) {
              $primerPaciente = $detallesPaciente[0];
          } else {
              continue; // Saltar a la siguiente iteración del bucle si no hay elementos en el array
          }
      } catch (Throwable $e) {
          // Acción alternativa cuando ocurre una excepción
      ...
      /*if($mesVisualizar>=12){
        ...
        ?>-<?php echo $cita->getAnnio() ?> ) </p>
            ...
            <a href="#" class="btn btn-primary btn-sm position-absolute top-0 end-0 translate-middle-y" data-bs-toggle="modal" data-titulo="<?php echo $cita->getTitulo() ?>" data-citanombre=<?php echo $cita->getNombrePaciente() ?> data-horafin=<?php echo $cita->getHoraFinal() ?> data-detalles="<?php echo $cita->getDetalles() ?>" data-horaprincipio=<?php echo $cita->getHoraInicio() ?> data-bs-target="#modalDetalleCita"><i class="fas fa-info-circle"></i></a>
          </li>
        </ul>
      <?php endforeach;
  ...
  <script>
    ...
    var tituloCita = button.getAttribute('data-titulo');
    var nombrepaciente = button.getAttribute('data-citanombre'); 
    var horaprincipio = button.getAttribute('data-horaprincipio');
    ...
    document.getElementById('tituloCita').innerHTML = tituloCita;
    document.getElementById('txtPacienteCita').innerHTML = "<strong>Paciente: </strong>"+ nombrepaciente;
    document.getElementById('horaCita').innerText = horaprincipio + "-" + horafin;
    document.getElementById('txtDetalleCita').innerHTML = "<strong>Detalles: </strong>" + detalles;
  });
  ...
  var fechasCitas = JSON.parse('<?php echo $jsonFechasCitas ?>');

  var celda = `<td class="text-center" id="celda${celdaBoton}" p-0 m-0 cell"><button type="button" data-bs-toggle="modal" id="celdaBoton" data-bs-target="#ModalEleccion"  onmouseout="this.style.backgroundColor='transparent'; this.style.color='inherit';" onmouseover="this.style.backgroundColor='#5c84a7'; this.style.color='#fff';" class="btn btn-transparent btn-hover w-100 h-100" onclick="subirVariableSesion(${dayOfMonth},${currentMonth},${currentDate.year()})">${dayOfMonth}</button></td>`;
              
  fechasCitas.forEach(function(fechaCita){
    console.log("El current month de prueba es: "+currentMonth);
    if(fechaCita.dia ==dayOfMonth && fechaCita.mes == currentMonth && fechaCita.annio == currentDate.year()){
      celda = `<td class="text-center bg-warning" onmouseout="this.style.backgroundColor='#5c84a7'; this.style.color='inherit';" onmouseover="this.style.backgroundColor='#ebb206.'; this.style.color='#fff';" id="celda${celdaBoton}" p-0 m-0 cell"><button type="button" data-bs-toggle="modal" id="celdaBoton" data-bs-target="#ModalEleccion"  class="btn btn-transparent fw-bold btn-hover w-100 h-100" onclick="subirVariableSesion(${dayOfMonth},${currentMonth},${currentDate.year()})">${dayOfMonth}</button></td>`;
    }
  });
  
  tableRows += celda;
              
  console.log("Ahora el currentMonth es: "+ currentMonth);
  dayOfMonth++;
}
